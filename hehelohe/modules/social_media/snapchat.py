...
def _marker_value(text, marker):
    """Extract a "marker<value>" token without raising IndexError."""
...
async def snapchat(email, client, out):
    ...
    if not xsrf or not webClientId:
        # Snapchat changed/omitted the login tokens: report rate limit
        # instead of crashing the whole scan with an IndexError.
